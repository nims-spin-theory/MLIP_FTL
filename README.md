...
>   3. Added support for building train/val/test sets from separate CSV files.  
>   4. Added a troubleshooting entry for `MemoryError` and `KeyError: 'edge_distance_vec'` errors in the warn file.
...
If training fails or stalls, you may see a `MemoryError` at the bottom of the warn file, or an error like:

```
[rank0]:   File "XXXXX/torch_geometric/data/storage.py", line 118, in __getitem__
[rank0]:     return self._mapping[key]
[rank0]: KeyError: 'edge_distance_vec'
```

**Solution**: Both errors can be caused by too many parallel data-loading workers. Reduce the number of workers by passing `--num_workers` with a smaller value (the default is 4). As a first step, try:

```bash
--num_workers 0
```

This disables parallel data loading and often resolves the issue.
